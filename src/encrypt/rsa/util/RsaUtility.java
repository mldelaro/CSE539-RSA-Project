/*
 * @FileName: RsaUtility.java
 * 
 * @Date: November 2015
 * @Author: Michael Bradley, Matthew de la Rosa
 * 
 * @Description: Use helper functions to perform mutations on byte blocks
 * 				and implement the mask generation function
 */
...
public final class RsaUtility {
	...
	/// @Param desiredByteLength - total number of blocks desired after
	/// appending
	public static byte[] appendZeroValueBytes(byte[] blockA, int desiredByteLength) {
		int oldByteLength = blockA.length;

		// Do not append blocks if the number of desired bytes is less then the
		// current length
	...
	public static byte[] concatenateByte(byte[] blockA, byte[] blockB) {
		int newBlockLength = blockA.length + blockB.length; // create a new
															// block
