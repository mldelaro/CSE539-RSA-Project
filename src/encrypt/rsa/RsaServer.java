...
public class RsaServer {
	...
	private BigInteger m_RandomPrime1; // p - used to create the public RSA
										// modulus
	private BigInteger m_RandomPrime2; // q - used to create the public RSA
										// modulus
	...
	public void receiveCiphertext(byte[] bytesCiphertext, boolean isPadded) {
		if (bytesCiphertext == null) {
			System.out.println("No ciphertext present");
			return;
		}
		...
		if (isPadded) {
			...
			byte[] HofX = null;
			try {
				MessageDigest hash512 = MessageDigest.getInstance("SHA-512"); // INSPECT
																				// SEED
																				// FOR
																				// THIS
																				// DIGEST
	...
	/// Recieve the incoming ciphertext and decrypt with the private key
	/// @Param bytesCiphertext - incoming ciphertext block
	public String receiveCiphertextWithErrorReturn(byte[] bytesCiphertext, boolean isPadded) {
		if (bytesCiphertext == null) {
			System.out.println("No ciphertext present");
		...
		if (isPadded) {
			...
			try {
				...
				System.out.println("Error");
				return "Ciphertext out of bounds";
			}
